...
on an image. The premise was tested before any pipeline was built; it survives, on one
statistic, and the test the track's central idea depends on came out flat enough to make
normalisation a gate rather than a refinement.
...
The epoch-comparison test is the one the track depends on, and it came out at 2.2\%:
within-field bottleneck 1{,}157 against between-field 1{,}183, WCS-aligned. Plate-to-plate
variation in emulsion, exposure and limiting magnitude is comparable to the difference between
one patch of sky and another. Measured directly on the plates used here, exposure times of
65~s and 132~s give source fractions of 0.005 and 0.059 --- an order of magnitude.

Normalisation is therefore a gate, not a refinement. Sublevel-set persistence depends only on
the order in which pixels enter the filtration, so a rank transform is the canonical
representative of the monotone class the method is already shape-invariant to, and it is the
principled correction rather than merely a conventional one. If even that fails to separate
same-field from different-field diagrams, the topology of these plates is dominated by grain
and defects whose ordering also varies plate to plate, no monotone correction can repair it,
and the temporal-topology premise fails.
